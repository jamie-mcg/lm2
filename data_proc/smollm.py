...
parser.add_argument(
    "-m", "--model_desc", type=str, default="llama-3", help="Model descriptor llama-3"
)
parser.add_argument(
    "-s",
    "--shard_size",
...
assert args.version in {
    "cosmo",
    "python",
    "fineweb",
    "smollm",
}, "version must be one of: cosmo, python, fineweb"
directories = {
...
def tokenize_llama(doc):
    ...
    assert (0 <= tokens_np).all() and (
        tokens_np < 2**32
    ).all(), "token dictionary too large for uint32"
    tokens_np_uint = tokens_np.astype(np.uint32)
    return tokens_np_uint


token_dtype = {"llama-3": np.uint32}[args.model_desc]
